...
public class OracleConnectionUtil {
	//오라클 연동하는 메소드를 작성. - 리턴 타입 Connection
...
//	DriverManager 클래스의 메소드 getConnection으로 데이터베이스 드라이버에 대한 Connection 구현객체를 생성
//													ㄴ인터페이스 Connection 타입으로 참조합니다.
			...
			System.out.println("데이터베이스 드라이버 로드에 문자가 생겼습니다. : "+e.getMessage());
		} catch (SQLException e) {
			System.out.println("데이터베이스 연결 및 사용에 문제가 생겼습니다. : " + e.getMessage());
		} 
		
		return conn;	// 생성된 Connection 객체를 리턴합니다.
	}
	
	//오라클 연결 Connection 객체 close 메소드 - 인자 타입 Connection
	public static void close(Connection conn) {
		if(conn != null)	//conn 이 null이 아닐때만 close 실행하도록 합니다.
			try {
				conn.close();
			} catch (SQLException e) {
